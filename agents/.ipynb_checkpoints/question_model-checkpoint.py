...
# Set HF_HOME to writeable directory to avoid read-only errors
os.environ["HF_HOME"] = "/tmp/hf_cache"
os.environ["HUGGINGFACE_HUB_CACHE"] = "/tmp/hf_cache"

class QAgent(object):
    def __init__(self, **kwargs):
        # User specified path or default
        # If user has model in /root/.cache/huggingface/models--Qwen--Qwen3-4B, we can try to use it directly
        # But 'models--Qwen--Qwen3-4B' is usually a dir with snapshots.
        # We'll stick to 'Qwen/Qwen3-4B' but with the env vars set, it should try to download to /tmp if not found.
        # However, user explicitly asked to "change dir to ...".
        # Let's try to use the model_name as "Qwen/Qwen3-4B" but ensure we use /tmp for cache.
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            model_name, 
            torch_dtype=torch.bfloat16, # Optimization matching A-Agent
            device_map="auto",
            cache_dir=cache_dir
        ).to("cuda") # Explicit CUDA move as per A-Agent optimization
