...
class QuestioningAgent(object):
    ...
    def build_inc_samples(self, inc_samples: List[Dict[str, str]] | None, topic: str) -> str:
        r"""
    ...
    def _normalize_question_obj(self, q: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize question object to strict competition format."""
        norm = dict(q)
        ...
        if isinstance(choices, dict):
            ...
            for idx, letter in enumerate(["A", "B", "C", "D"]):
                value = str(choices.get(letter, "")).strip()
                value = re.sub(r"^[A-Da-d]\)\s*", "", value)
                normalized_choices.append(f"{chr(65 + idx)}) {value}".strip())
            norm["choices"] = normalized_choices
        ...
        answer = str(norm.get("answer", "")).strip().upper()
        if answer:
            for ch in answer:
                if ch in "ABCD":
                    norm["answer"] = ch
    ...
    def generate_question(
        ...
        **gen_kwargs,
    ) -> Tuple[List[str] | str, float | None, float | None]:
        ...
        while pending_indices and attempts < max_attempts:
            ...
            for idx_in_batch, original_idx in enumerate(pending_indices):
                ...
                try:
                    final_tls[original_idx] = float(tls[idx_in_batch])
                except (TypeError, ValueError):
                    final_tls[original_idx] = 0.0
                try:
                    final_gts[original_idx] = float(gts[idx_in_batch])
                except (TypeError, ValueError):
                    final_gts[original_idx] = 0.0
                ...
                try:
                    if cleaned:
                        obj = self._normalize_question_obj(json.loads(cleaned))
                ...
                if is_valid:
                    final_responses[original_idx] = json.dumps(obj, ensure_ascii=False)
                else:
    ...
    def count_tokens_q(self, text: str) -> int:
        """Count the number of tokens using the agent's tokenizer (matches official oracle)"""
    ...
    def validate_question(self, q2: Dict[str, Any]) -> bool:
        """Official oracle filter: basic_checks for question format.
        Matches the competition judging system exactly."""
        if not isinstance(q2, dict):
            return False

        # check required keys (official: topic, question, choices, answer)
        required_keys = ['topic', 'question', 'choices', 'answer']
        if not all((key in q2) for key in required_keys):
            return False

        # check choices format (official: list of 4, each len>2, first char in ABCD)
        choices = q2['choices']
        ...
        if not isinstance(q2['answer'], str):
            return False

        # Token length check (official formula):
        # sum(tokens(question, answer)) + sum(tokens(choices)) - 15 < 130
        ...
        # Total with explanation must be <= 1024
    ...
    def filter_questions(
        ...
        """Official oracle filter_questions — returns empty list if <50% survive."""
        correct_format_question = []
        for i, q in enumerate(questions):
            if isinstance(q, dict):
                q1 = self._normalize_question_obj(q)
                if self.validate_question(q1):
                    correct_format_question.append(q1)
            elif isinstance(q, str):
                try:
                    q1 = self._normalize_question_obj(json.loads(q))
                    if self.validate_question(q1):
                        correct_format_question.append(q1)
                except json.JSONDecodeError:
                    print(f"Skipping invalid JSON at index {i}: {q[:50] if isinstance(q, str) else q}")
                    continue
            else:
                continue
        # Official rule: if fewer than 50% pass, return empty list
...
if __name__ == "__main__":
    ...
    if args.verbose:
        ...
        if gen_kwargs.get("tgps_show", False):
            total_gts = sum(x for x in gts if isinstance(x, (int, float)))
            total_tls = sum(x for x in tls if isinstance(x, (int, float)))
            print("Time taken per batch generation:", gts)
            print("Tokens generated per batch:", tls)
            print(
                f"Total Time Taken: {total_gts:.3f} seconds; Total Tokens: {int(total_tls)}; TGPS: {(total_tls/total_gts if total_gts > 0 else 0):.3f} seconds\n\n"
            )
        print("\n" + "+" * 50 + "\n")
    ...
    for q in question:
        try:
            ...
            q, _tl, _gt = agent.agent.generate_response(
                prompt.format(q),
